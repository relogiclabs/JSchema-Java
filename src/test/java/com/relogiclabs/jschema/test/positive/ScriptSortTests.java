        ...
        var schema =
            ...
            %script: {
                constraint mergeSort(result) {
                    ...
                    mergeSort(array, 0, target.length() - 1);
                    if(array != result) return fail("Invalid: " + target);
                }
        ...
        var schema =
            ...
            %script: {
                constraint quickSort(result) {
                    var array = target.copy();
                    quickSort(array, 0, target.length() - 1);
                    if(array != result) return fail("Invalid: " + target);
                }
        ...
        var schema =
            ...
            %script: {
                ...
                subroutine radixSort(array) {
                    ...
                    var numDigits = (max.log() / 10.log()).floor() + 1;
                    for(var digit = 1; digit <= numDigits; digit++) {
                        var buckets = [];
                        for(var i = 0; i < 10; i++) buckets[i] = [];

                        foreach(var num in array) {
                            var bucket = buckets[((num / 10.pow(digit - 1)) % 10).floor()];
                            bucket[bucket.length()] = num;
                        }
                        var index = 0;
